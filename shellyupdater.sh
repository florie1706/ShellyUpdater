...
for SHELLYIP in $(avahi-browse -d local -k -v -t -r -p _http._tcp | grep helly | grep 192 | cut -d ';' -f8 | awk '{ print length(), $0 | "sort -n" }' | cut -d ' ' -f2)
	do
	UPDATE=0
...
#### check if newer version is availible and if there is a newer version then your choosen trunk on STABLE/PRE when PRE/BETA was selected) for new shipped devices the current firmware will be installed ####
		...
		OLDFIRMWARERC=$(echo $OLDFIRMWAREFULL | cut -d 'v' -f2 | cut -d '@' -f1 | cut -d '-' -f2)
		if [ $OLDFIRMWARERC != $OLDFIRMWARE ] ; then
			OLDFIRMWARE=$(echo $OLDFIRMWARE-$OLDFIRMWARERC)
		fi
		if [ $UPDATE != 0 ] ; then
			...
			NEWFIRMWARERC=$(echo $NEWFIRMWAREFULL | cut -d 'v' -f2 | cut -d '@' -f1 | cut -d '-' -f2)
			if [ $NEWFIRMWARERC != $NEWFIRMWARE ] ; then
				NEWFIRMWARE=$(echo $NEWFIRMWARE-$NEWFIRMWARERC)
			fi
